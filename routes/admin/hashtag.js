...
router.get('/:hashtag_id', async (req, res) => {
    if(! await hashtag_model.ifExists("hashtags", "hashtag_id", req.params.hashtag_id)){
...
router.post('/', async (req, res) => {
    ...
    if(await hashtag_model.ifExists("hashtags", "hashtag_nama", nama)){
...
router.put('/:hashtag_id', async (req, res) => {
    ...
    const { nama } = req.body;
    if(! await hashtag_model.ifExists("hashtags", "hashtag_id", hashtag_id)){
        return res.status(404).send({error: "Hashtag not found"});
    }
    if(await hashtag_model.ifExists("hashtags", "hashtag_nama", nama)){
...
router.delete('/:hashtag_id', async (req, res) => {
    ...
    if(await hashtag_model.ifExists("hashtag_artikel", "hashtag_id", hashtag_id)){
        return res.status(404).send({error: "Hashtag can't be deleted because it is used in some artikels"});
    }
    if(await hashtag_model.ifExists("hashtag_comment", "hashtag_id", hashtag_id)){
